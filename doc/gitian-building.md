...
*Setup instructions for a Gitian build of PAC Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the PAC
...
- [Building PAC Core](#building-pac-core)
...
Clone the git repositories for PAC Core and Gitian.
...
in the PAC Core repository under 'Fetch and create inputs' to install sources which require
...
To build PAC Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the PAC Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/PAC/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/PACCommunity/PAC
...
./bin/gbuild --commit PAC=${COMMIT} --url PAC=${URL} ../PAC/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit PAC=${COMMIT} --url PAC=${URL} ../PAC/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit PAC=${COMMIT} --url PAC=${URL} ../PAC/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the PAC git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../PAC/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/PAC
SIGPATH=/some/root/path/PAC-detached-sigs

./bin/gbuild --url PAC=${BTCPATH},signature=${SIGPATH} ../PAC/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/PAC-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/PAC-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/PAC-osx-build.assert
```
...
[PAC/gitian.sigs](https://github.com/PACCommunity/gitian.sigs/) repository, or if that's not possible to create a pull
request.
