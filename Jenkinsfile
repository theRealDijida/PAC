...
// which are part of the PAC repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="paccoin-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /pac-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "paccommunity-pac/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from paccommunity-pac/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'paccommunity-pac/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from paccommunity-pac/develop"
            }
          }

          if (hasCache) {
            sh "cd /pac-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /pac-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /pac-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /pac-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /pac-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /pac-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /pac-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
